...
records/tests.py
─────────────────
Tests for FinancialRecord CRUD, soft delete, scoped data, and RBAC.
"""
...
class FinancialRecordModelTest(APITestCase):
    ...
    def test_default_ordering_newest_first(self):
        make_record(self.user, date=date.today() - timedelta(days=10), title="Older")
        make_record(self.user, date=date.today() - timedelta(days=1), title="Newer")
...
class RecordRBACTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.admin_record = make_record(self.admin, title="Admin Record")
        self.analyst_record = make_record(self.analyst, title="Analyst Record")

        self.list_url = reverse("record-list-create")
        self.admin_detail = reverse("record-detail", kwargs={"pk": self.admin_record.pk})
        self.analyst_detail = reverse("record-detail", kwargs={"pk": self.analyst_record.pk})
    ...
    def test_analyst_sees_only_own_records_in_list(self):
        resp = self.analyst_c.get(self.list_url)
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        titles = {r["title"] for r in resp.data["results"]}
        self.assertIn("Analyst Record", titles)
        self.assertNotIn("Admin Record", titles)

    def test_admin_sees_all_records_in_list(self):
        resp = self.admin_c.get(self.list_url)
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        titles = {r["title"] for r in resp.data["results"]}
        self.assertIn("Admin Record", titles)
        self.assertIn("Analyst Record", titles)
    ...
    def test_admin_can_create(self):
        resp = self.admin_c.post(self.list_url, self._create_payload(title="Created By Admin"))
    ...
    def test_analyst_can_retrieve_own_record(self):
        resp = self.analyst_c.get(self.analyst_detail)
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(resp.data["data"]["title"], "Analyst Record")

    def test_analyst_cannot_retrieve_admin_record(self):
        resp = self.analyst_c.get(self.admin_detail)
        self.assertEqual(resp.status_code, status.HTTP_404_NOT_FOUND)

    def test_viewer_cannot_retrieve(self):
        resp = self.viewer_c.get(self.analyst_detail)
        self.assertEqual(resp.status_code, status.HTTP_403_FORBIDDEN)

    def test_analyst_cannot_update(self):
        resp = self.analyst_c.patch(self.analyst_detail, {"title": "Hacked"})
        self.assertEqual(resp.status_code, status.HTTP_403_FORBIDDEN)

    def test_admin_can_update(self):
        resp = self.admin_c.patch(self.admin_detail, {"title": "Updated by Admin"})
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(resp.data["data"]["title"], "Updated by Admin")

    def test_viewer_cannot_delete(self):
        resp = self.viewer_c.delete(self.admin_detail)
        self.assertEqual(resp.status_code, status.HTTP_403_FORBIDDEN)

    def test_analyst_cannot_delete(self):
        resp = self.analyst_c.delete(self.analyst_detail)
        self.assertEqual(resp.status_code, status.HTTP_403_FORBIDDEN)

    def test_admin_soft_delete_returns_204(self):
        rec = make_record(self.admin, title="To Delete")
        url = reverse("record-detail", kwargs={"pk": rec.pk})
        resp = self.admin_c.delete(url)
        self.assertEqual(resp.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(FinancialRecord.objects.filter(pk=rec.pk).exists())
        self.assertTrue(FinancialRecord.all_records.get(pk=rec.pk).is_deleted)

    def test_unauthenticated_cannot_delete(self):
        resp = self.anon_c.delete(self.admin_detail)
        self.assertEqual(resp.status_code, status.HTTP_401_UNAUTHORIZED)
